...
function updateOpenClaw() {
  ...
  const proc = runInProotWithCallback('npm install -g git+https://github.com/openclaw/openclaw.git', () => {
...
export async function main(args) {
  if (process.env.NODE_OPTIONS && process.env.NODE_OPTIONS.includes('bionic-bypass')) {
    console.error('\n❌ Error: OpenClaw Wrapper running inside container!');
    console.error('The "openclaw" command inside Ubuntu is pointing to the Termux wrapper.');
    console.error('This prevents the Gateway from starting.\n');
    console.error('To fix this, run inside Ubuntu:');
    console.error('  npm uninstall -g openclaw');
    console.error('  npm install -g git+https://github.com/openclaw/openclaw.git\n');
    process.exit(1);
  }
