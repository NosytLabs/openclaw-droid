...
export async function installUbuntu() {
  console.log('Installing Ubuntu in proot...');
  return new Promise((resolve) => {
    const child = spawn('proot-distro', ['install', 'ubuntu'], { stdio: 'inherit' });
    ...
    child.on('close', (code) => {
      if (code === 0) {
        ...
        console.error('Failed to install Ubuntu: Process exited with code', code);
        resolve(false);
      }
    });
  });
