...
function checkCommand(cmd) {
  try {
    ...
    if (err.status !== 127 && err.status !== 1 && err.code !== 'ENOENT') {
      console.warn(`Warning: Check '${cmd}' failed with status ${err.status}: ${err.message}`);
    }
    return false;
  }
}
...
export function checkDependencies() {
  ...
  deps.node = checkCommand('node --version');
  deps.npm = checkCommand('npm --version');
  deps.git = checkCommand('git --version');
  deps.proot = checkCommand('command -v proot-distro');
...
export function getInstallStatus() {
  ...
  const hasProot = checkCommand('command -v proot-distro');
  ...
  return {
    ...
    openClaw: checkCommand('command -v openclaw')
  };
}
